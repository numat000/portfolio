...
(function () {
  ...
  if (morphStage && morphPath && morphTarget && morphBlackout) {
    ...
    morphTL
      .to(morphStage, { opacity: 1, duration: 0.15, ease: 'power2.out' })
      .to(morphPath, {
      ...
      .to(morphStage, { opacity: 0, duration: 0.05 });
  }
    ...
    question.addEventListener('click', () => {
      ...
      faqItems.forEach((o) => {
        if (o !== item && o.classList.contains('is-open')) {
          ...
          gsap.to(o.querySelector('.faq-answer'), { height: 0, opacity: 0, duration: 0.35, ease: 'power2.inOut' });
        }
      });
      if (isOpen) {
        ...
        gsap.to(answer, { height: 'auto', opacity: 1, duration: 0.4, ease: 'power2.inOut' });
      }
    });
  });
  gsap.from('.faq-item', { opacity: 0, y: 20, duration: 0.5, stagger: 0.1, ease: 'power2.out',
    scrollTrigger: { trigger: '.faq-list', start: 'top 75%' } });

  /* 16. Contact */
  gsap.from('.contact-text', { opacity: 0, y: 30, duration: 0.8, ease: 'power3.out',
    scrollTrigger: { trigger: '.contact-content', start: 'top 75%' } });
  gsap.from('.contact-btn', { opacity: 0, y: 20, duration: 0.6, ease: 'power3.out',
  ...
  if (isMobile) {
    ScrollTrigger.getAll().forEach((st) => {
      if (st.pin === worksSection) st.kill();
    });
    if (worksTrack) {
      gsap.set(worksTrack, { x: 0 });
      const ws = document.querySelector('.works-horizontal-scroll');
      if (ws) { ws.style.overflowX = 'auto'; ws.style.WebkitOverflowScrolling = 'touch'; }
    }
  }
  ...
  mobileNav.innerHTML = '<nav class="mobile-nav-content"><ul class="mobile-nav-list"><li><a href="#works" class="mobile-nav-link">Works</a></li><li><a href="#service" class="mobile-nav-link">Service</a></li><li><a href="#voice" class="mobile-nav-link">Voice</a></li><li><a href="#contact" class="mobile-nav-link">Contact</a></li></ul></nav>';
  document.body.appendChild(mobileNav);

  if (navToggle) {
    navToggle.addEventListener('click', () => {
      isNavOpen = !isNavOpen;
      if (isNavOpen) { mobileNav.classList.add('is-open'); navToggleText.textContent = 'Close'; lenis.stop(); }
      else { mobileNav.classList.remove('is-open'); navToggleText.textContent = 'Menu'; lenis.start(); }
    });
  }
  mobileNav.querySelectorAll('.mobile-nav-link').forEach((link) => {
    link.addEventListener('click', (e) => {
      ...
      if (target) setTimeout(() => { lenis.scrollTo(target, { offset: -80 }); }, 300);
    });
  });
